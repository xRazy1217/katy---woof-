...
require_once __DIR__ . '/../schema-definition.php';
    ...
    public function auditSchema() {
        ...
        foreach ($this->idealSchema as $tableName => $tableSchema) {
            ...
            if ($tableReport['status'] === 'OK') {
                ...
                if ($tableReport['status'] === 'MISSING_TABLE') {
                    $report['missing_tables_count']++;
                }

                $report['missing_columns_count'] += count($tableReport['missing_columns']);
            }
        }
    ...
    public function syncDatabase() {
        ...
        try {
            ...
            if (empty($sqlStatements)) {
                $this->pdo->commit();
                $result['success'] = true;
                $result['message'] = 'No hay cambios por aplicar. El esquema ya est\u00e1 sincronizado.';
                return $result;
            }
